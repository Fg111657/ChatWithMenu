...
function ChatScreen() {
      ...
      try {
        ...
        const normalizedItems = Array.isArray(data?.items)
          ? data.items
              ...
              .filter(Boolean)
          : [];
        setMenuItemsFromChat(normalizedItems);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  // Load or start chat
  useEffect(() => {
    if (!isInitialized || !restaurantId || !userId) {
      return;
    }
    ...
    const fetchChatData = async () => {
      try {
        ...
        console.log(error);
        setAlert({ severity: 'error', message: error.message || 'Unable to start chat right now.' });
      }
    };
    fetchChatData();
  }, [isInitialized, userId, restaurantId]);

  const sendMessage = async (inputMsg) => {
    if (!restaurantId) {
      setAlert({ severity: 'error', message: 'Choose a restaurant before starting a chat.' });
      return;
    }
    if (!userId) {
      setAlert({ severity: 'error', message: 'Please sign in to use chat.' });
      return;
    }
    if (chatId < 0) {
      setAlert({ severity: 'error', message: 'Chat is still loading. Please try again in a moment.' });
      return;
    }
    ...
    try {
      ...
      console.log(error);
      setLoading(false);
      setAlert({ severity: 'error', message: error.message || 'Unable to send message right now.' });
    }
  };
  ...
  const placeOrder = () => {
    ...
    setAlert({ severity: 'success', message: 'Order sent to kitchen! 🍳' });
    setOrderItems([]);
  };
  ...
  const newChat = async () => {
    ...
    try {
      ...
      console.log(error);
      setAlert({ severity: 'error', message: error.message || 'Unable to start a new chat right now.' });
    }
  };
  ...
  const addRating = async ({ menuItem, rating, extraText }) => {
    try {
      ...
      setAlert({ severity: 'success', message: 'Review saved!' });
    } catch (error) {
      setAlert({ severity: 'error', message: error.message || 'Unable to save review right now.' });
    }
  };
  ...
  const loadAndShowReviews = async () => {
    if (!restaurantId || !userId) {
      setAlert({ severity: 'error', message: 'Choose a restaurant before editing reviews.' });
      return;
    }
      ...
      <Container maxWidth="sm" sx={{ py: 8 }}>
        <Paper elevation={2} sx={{ p: 4, textAlign: 'center' }}>
          ...
          <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
            Chat uses your profile and dietary preferences, so please sign in first.
          </Typography>
    ...
    return (
      <Container maxWidth="sm" sx={{ py: 8 }}>
        <Paper elevation={2} sx={{ p: 4, textAlign: 'center' }}>
          ...
          <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
            Open chat from a restaurant card so we know which menu to help with.
          </Typography>
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 64px)' }}>
      ...
      <AddReviewModal
        ...
        onSubmit={addRating}
        suggested={menuItemsFromChat.map((item) => item.name || item.title).filter(Boolean)}
      />
      <EditReviewsModal
      ...
      >
        <Alert severity={alert?.severity || 'info'} sx={{ width: '100%' }}>
          {alert?.message || ''}
        </Alert>
      </Snackbar>
